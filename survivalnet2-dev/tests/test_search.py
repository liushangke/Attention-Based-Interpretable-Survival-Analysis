...
def test_keras_optimizer():
    ...
    assert (
        str(exc.value) == "optimizer['method'] must be one of 'adadelta', 'adam', 'adagram', 'rms', or 'sgd'"
    )
...
def load_example(batch_size):
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices((features[train, :],
                                                        {"osr": osr[train, :],
                                                         "pfi": osr[train, :]}))
    train_dataset = train_dataset.shuffle(sum(train), reshuffle_each_iteration=True)
    train_dataset = train_dataset.batch(batch_size)
    validation_dataset = tf.data.Dataset.from_tensor_slices((features[~train, :],
                                                             {"osr": osr[~train, :],
                                                              "pfi": pfi[~train, :]}))
    validation_dataset = validation_dataset.shuffle(train.size - sum(train), reshuffle_each_iteration=True)
...
def test_search():
    ...
    # test experiment
    space = advmtl_space(412,
                         tasks={"osr": task_space(loss = {'efron', 'cox'}),
                                "pfi": task_space(loss = {'efron', 'cox'})})
    space["epochs"] = 10
    tuner = Search(max_concurrent_trials=1, num_samples=1)
    results = tuner.experiment(metric = "osr_harrellsc",
                               space = space,
                               builder = advmtl_model,
                               loader = load_example,
                               local_dir = ROOT_TEST_DIR)
    ...
    def sample(config):
        for k in config.keys():
            ...
            elif isinstance(config[k], (ray.tune.search.sample.Integer,
